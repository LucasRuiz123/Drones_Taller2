...
def backtracking(csp: DroneAssignmentCSP, assignment):
    if csp.is_complete(assignment):
        return assignment
    var = csp.get_unassigned_variables(assignment=assignment)[0]
...
def AC_3(csp, queue=None):
    if queue is None:
        queue = [(Xi, Xj) for Xi in csp.variables for Xj in csp.get_neighbors(Xi)]
    while queue:
        Xi, Xj = queue.pop()
        if revise(csp, Xi, Xj):
